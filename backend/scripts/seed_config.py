"""seed_config.py — Siembra de datos de configuración del maestro Dupon.

Siembra SOLO lo que no viene de Odoo (o configuración propia de fábrica):
  - 5 Compañías del grupo (con odoo_company_id reales confirmados)
  - 5 Silos de Ibérica (con odoo_location_id y odoo_product_id reales)
  - 10 Líneas de producción (capacidades teóricas kg/h)
  - Tabla CON: consumos de referencia por receta/formato (Excel CON, kg/día por máquina)
  - Mappings Formatos por línea con nº máquinas
...
schema_prefix = "" if _IS_SQLITE else "dcp_app."

# ── 1. Limpiar solo tablas de configuración ───────────────────────────────────
CONFIG_TABLES = [
    "line_formats",
...
print("🧹 Limpiando tablas de configuración...")
for tbl in CONFIG_TABLES:
    try:
        db.execute(text(f"DELETE FROM {schema_prefix}{tbl}"))
    except Exception as e:
        print(f"   Nota al limpiar {tbl}: {e}")
db.commit()
print("✅ Tablas de config preparadas (usuarios, stock y réplica Odoo preservados)")

# ── 2. Compañías ──────────────────────────────────────────────────────────────
companies = [
    Company(id=1, name="Dupon Biscuits Ibérica, S.A.", odoo_company_id=1, short_code="IBE", is_active=True),
    Company(id=2, name="Dupon Deutschland GmbH",       odoo_company_id=8, short_code="GUD", is_active=True),
    Company(id=3, name="Biscuits Dupon France",        odoo_company_id=3, short_code="FRA", is_active=True),
    Company(id=4, name="Biscotti Dupon Italia",        odoo_company_id=4, short_code="ITA", is_active=True),
    Company(id=5, name="Biscuits Dupon Belgium",       odoo_company_id=5, short_code="BEL", is_active=True),
]
for c in companies:
...
# ── 3. Silos Ibérica ─────────────────────────────────────────────────────────
# IDs confirmados en Odoo staging3:
# Harina=49486, Azúcar=49488, Aceite=49492
# Silo 1=79, Silo 2=80, Silo 3=81, Azúcar=82, Aceite=83
silos = [
    SiloConfig(
        silo_code="S-H1",
        name="Silo Harina 1",
    ...
    SiloConfig(
        silo_code="S-AZ",
        name="Silo Azúcar",
        material_type="azucar",
        capacity_kg=30000,
        safety_stock_kg=2500,
        company_id=1,
        odoo_location_id=82,
        odoo_product_id=49488,
    ),
    SiloConfig(
        silo_code="S-AC",
        name="Silo Aceite",
        material_type="aceite",
        capacity_kg=20000,
        safety_stock_kg=1500,
        company_id=1,
        odoo_location_id=83,
        odoo_product_id=49492,
    ),
]
db.add_all(silos)
db.commit()
print(f"✅ Silos Ibérica: {len(silos)}")

# ── 4. Líneas de producción Ibérica (Capacidad teórica kg/h) ──────────────────
line_data = [
    ("L01", 450),
    ("L02", 450),
    ("L03", 380),
...
db.add_all(lines)
db.commit()
print(f"✅ Líneas de galleta (L01-L10 con capacidades kg/h): {len(lines)}")

# ── 5. Tabla CON — Consumos de referencia (kg/día por máquina 24h) ───────────
# Fuente: hoja CON de Apro-PM 2026.ods (Ibérica, company_id=1)
CON_DATA = [
    ...
    # ── MINI_75_OLI (Mini 75mm con baño de aceite)
...
db.commit()
print(f"✅ Recetas / Consumos CON: {len(con_records)} registros")

# ── 6. Formatos por línea ─────────────────────────────────────────────────────
LINE_FORMAT_DATA = [
    # Rotativas: L01, L02, L06 — múltiples cabezales (2 o 3 máquinas equivalentes)
    ("L01", "STD_R_110",    2),
    ("L01", "STD_R_SS",     2),
    ("L02", "STD_R_110",    2),
    ("L02", "STD_R_SS",     2),
...
db.commit()
print(f"✅ Formatos por línea (Líneas - Formatos): {len(lf_records)} mappings")

db.close()
print("\n🎉 Siembra de configuración de fábrica completada con éxito.")
